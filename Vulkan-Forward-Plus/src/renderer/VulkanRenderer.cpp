...
void _VulkanRenderer_Impl::updateUniformBuffers(float deltatime)
{
	...
	// update light ubo
	{
        ...
        util::writeLog("update ubo --> camera ubo: map => copy => unmap, copy staging => destination\n "
            "update ubo --> light ubo: map => copy => unmap, copy staging => destination\n ");
}
...
void _VulkanRenderer_Impl::drawFrame()
{
    ...
    util::writeLog("draw a frame --> acquire an image from swap chain.[wait swapchain image available]\n "
