...
protected:
	...
	// Overlay: Render to final render target: BackBuffer
    void RenderClusterVisualizer(); // prepare to remove
	void RenderLightClusterVisualizer();
    void RenderHUD(const Timer& timer);

    // We're still waiting for shaders to compile, so print a message to the screen
	// Overlay: Render to final render target: BackBuffer
	void RenderTipsForShaderCompiling(const Timer& timer);


	// Thread work for multi-thread compiling
