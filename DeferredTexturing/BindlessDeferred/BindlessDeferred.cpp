...
// We're still waiting for shaders to compile, so print a message to the screen
void BindlessDeferred::RenderTipsForShaderCompiling(const Timer& timer)
{
...
void BindlessDeferred::Render(const Timer& timer)
{
    ...
    if(taskSet != nullptr)
    {
		...
		RenderTipsForShaderCompiling(timer);
        return;
    }
	...
	// Render into final target: back buffer
    RenderLightClusterVisualizer();
    RenderHUD(timer);
}
...
// Overlay: Render into Final Target: Back buffer
void BindlessDeferred::RenderLightClusterVisualizer()
{
    if(AppSettings::ShowClusterVisualizer == false) return;
