...
vec4 myShade( vec4 lightPos, vec3 lightColor, vec4 surfaceColor, vec3 surfacePos, vec3 surfaceNormal, vec3 eyePos )
{
    vec3 L, H, V, R, N;
    L = normalize( lightPos.xyz - surfacePos );
...
void main()
{
    ...
    {
        out_Color = vec4( textureProj(u_shadowmap, shadowCoord.xyw) );
        return;
    }
    else
    {
        ...
        {
            //out_Color = color * vec4( myShade(u_LightPos, u_LightColor, color, pos.xyz, normal, u_eyePos).xyz, 1.0f );
            out_Color = myShade(u_LightPos, u_LightColor, color, pos.xyz, normal, u_eyePos);
        }
    }
}
...
void temp()
{
    ...
    out_Color = visibility * vec4( shade( u_LightPos, u_LightColor, color, posEyeSpace.xyz/posEyeSpace.w, normal ), 0.0f ); 
