...
{    
    ...
    float3 avg = 0.25f * ( ip[5].vPosition+ip[6].vPosition+ip[9].vPosition+ip[10].vPosition );
    float d = distance(avg, g_vCameraPosWorld);
    float s = saturate( (d - 0.1f) / (600.f - 0.f) );
    TessAmount = pow(2, (lerp(6.f, 0.f, s)));
...
// The tessellation factor, topology, and partition mode will go to the fixed function Tessellator
// tessellator HW to calculate the UVW and domain points.
///////////////////////////////////////////////////////////////////////////////////////////////////////////
[domain("quad")]
